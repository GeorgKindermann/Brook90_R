## ----chunk4--------------------------------------------------------------
meteoFile <-read.table(file.path(dataPath,"data_input_WB_1999_point.txt"))
MData<-matrix(meteoFile)

precFile <-read.table(file.path(dataPath,"P_WBG_0060_Minuten_1999_point.txt"))
MhhData<-matrix(precFile)


## ----chunk5, warning=FALSE-----------------------------------------------
canopy_file <- read.table(file.path(dataPath,"canopy.txt"), header = FALSE, fill=TRUE , dec = "." , sep = ",", strip.white = TRUE)
...
rootden <- lapply(strsplit(canopy_varname[20:23],"[ ]+"), function(x) {as.numeric(sub("^\\.$", "0", x))})
rootden <- as.vector(rbind(c(rootden[[1]], rootden[[3]]), c(rootden[[2]], rootden[[4]])))
assign(canopy_varname[19],rootden)
rm(quant_canopy,canopy_varvalue,canopy_varname,rootden)

## ----chunk6--------------------------------------------------------------
fixed_file <-read.table(file.path(dataPath,"fixed.txt"), header = FALSE, fill=TRUE, sep = "," , dec = "." , row.names = 1)
...
## ----chunk7--------------------------------------------------------------
flow_file <-read.table(file.path(dataPath,"flow_standard.txt"), header = FALSE, fill=TRUE, sep = "," , dec = "." , row.names = 1)
...
## ----chunk8--------------------------------------------------------------
initial_file <-read.table(file.path(dataPath,"initial.txt"),header = FALSE,fill=TRUE,sep = ",",dec = ".",row.names = 1)
...
## ----chunk9--------------------------------------------------------------
location_file <-read.table(file.path(dataPath,"location.txt"), header = FALSE, fill=TRUE, sep = "," , dec = ".", strip.white = TRUE)
...
## ----chunk10-------------------------------------------------------------
soil_file <-read.table(file.path(dataPath,"soil.txt"), header = FALSE, fill=TRUE , dec = "." , sep = "", strip.white = TRUE)
